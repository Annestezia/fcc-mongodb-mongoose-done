...
//     name: 'Vlad', 
//     age: 589,
//     favoriteFoods:['blood', 'meat', 'galic']
//   });
...
//   });
// };
// createAndSavePerson((err, data ) =>{ if(err){console.log(err);} console.log(data);})
...
var queryChain = function(done) {
  ...
  Person.find({favoriteFoods:foodToSearch}).sort('asc name').limit(2).select('-age').exec((err,data)=>{
       if(err) return done(err);
    done(null,data); 
  });
  // done(null/*, data*/);
